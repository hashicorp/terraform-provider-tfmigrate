...
var (
	...
	ErrTfGitPatTokenEmpty                = GitTokenError(`%s environment variable is empty`)
	ErrTfGitPatTokenNotSet               = GitTokenError(`%s environment variable not set`)
	...
	ErrTfGitPatTokenFineGrained          = GitTokenError(`%s is a Github fine-grained token`)
	ErrTfGitPatTokenValid                = GitTokenError(`%s is valid`)
	ErrTfGitPatTokenInvalid              = GitTokenError(`%s is invalid`)
	ErrTokenDoesNotHaveAccessToRepo      = GitTokenError(`the provided git token does not have access to the repository`)
	ErrTokenDoesNotHavePrWritePermission = GitTokenError(`the provided git token does not have permission to create pull requests`)
	ErrBitbucketTokenTypeNotSupported    = GitTokenError(`%s is not a valid Bitbucket Repository Access Token`)
